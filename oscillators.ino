void calculate_oscillators_orig(oscillators &timings) {
...
void calculate_oscillators_slow(oscillators &timings) {
  float prox_scaled, proxb_scaled;
  float prox = min(proximity, 350.0);
  float proxb = min(proximityb, 350.0);
  prox = max(30, prox);
  ...
  for (int i = 0; i < num_oscillators; i++) {
    move.linear[i]      = (runtime + timings.offset[i]) * timings.ratio[i] * (prox_scaled * proxb_scaled);     // continously rising offsets, returns              0 to max_float
    move.radial[i]      = fmodf(move.linear[i], 2 * PI);                        // angle offsets for continous rotation, returns    0 to 2 * PI
    move.directional[i] = sinf(move.radial[i]);                                 // directional offsets or factors, returns         -1 to 1
    move.noise_angle[i] = PI * (1 + pnoise(move.linear[i], 0, 0));              // noise based angle offset, returns                0 to 2 * PI
  }
...
void calculate_oscillators_sloth(oscillators &timings) {
  float prox_scaled, proxb_scaled;
  float prox = min(proximity, 350.0);
  float proxb = min(proximityb, 350.0);
  prox = max(30, prox);
  ...
  for (int i = 0; i < num_oscillators; i++) {
    move.linear[i]      = (runtime + timings.offset[i]) * timings.ratio[i] * ((prox_scaled * proxb_scaled) / 2);     // continously rising offsets, returns              0 to max_float
    move.radial[i]      = fmodf(move.linear[i], 2 * PI);                        // angle offsets for continous rotation, returns    0 to 2 * PI
    move.directional[i] = sinf(move.radial[i]);                                 // directional offsets or factors, returns         -1 to 1
    move.noise_angle[i] = PI * (1 + pnoise(move.linear[i], 0, 0));              // noise based angle offset, returns                0 to 2 * PI
  }
